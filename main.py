...
@app.get("/{user_id}/notifications/", response_model=list[schemas.Notification])
def get_user_notifications(user_id: int, db: Session = Depends(get_db)):
    user_notifications = db.query(models.Notification).filter(models.Notification.user_id == user_id).all()

    if not user_notifications:
        raise HTTPException(status_code=404, detail=f"User with id {user_id} does not have any notifications.")
    
    return user_notifications

@app.post("/goals/new-goal/", response_model=schemas.Goal)
...
@app.put("/goals/{goal_id}/update-goal/", response_model=schemas.Goal)
...
@app.delete("/goals/{goal_id}/delete-goal/")
...
@app.get("/{user_id}/goals/", response_model=list[schemas.Goal])
...
@app.post("/create-progress/", response_model=schemas.UserProgress)
...
@app.put("/{user_id}/update-progress/", response_model=schemas.UserProgress)
...
@app.delete("/{user_id}/clear-progress/")
...
@app.get("/{user_id}/progress/", response_model=schemas.UserProgress)
